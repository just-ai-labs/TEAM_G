...
const handleGitHubError = (error: any): never => {
  ...
  if (error.status === 403) {
    throw new Error('API rate limit exceeded or insufficient permissions. Please check your token permissions.');
  }
  ...
  throw new Error(error.message || 'An unexpected error occurred while fetching repository data');
};
